			...
			switch(type){
				...
				case Sundry.events.ROOMINFO:
					...
					if(_room){
						...
						_event.setStatus(false).setDescribe(Sundry.describes.UNEXIST).assignData({room:_roomid});
					}
					fn&&fn(_event);
					socket.send(_event.info);
					break;
				case Sundry.events.CREATEROOM:
					...
					if(_room){
						/*创建房间成功*/
						_event.setStatus(true).setDescribe(Sundry.describes.SUCCESS).assignData({room:_room.id});
						/*是否自动加入到新创建的房间*/
						...
						/*创建房间被限制*/
						_event.setStatus(false).setDescribe(Sundry.describes.LIMITED).assignData({amount:Room.rooms.length});
					}
					fn&&fn(_event);
					socket.send(_event.info);
					break;
				...
				case Sundry.events.ROOMSTATE:
					...
					if(_.has(data,'room')&&_.has(data,'state')){
						...
						if(_room){
							...
							_event.setStatus(true).setDescribe(Sundry.describes.SUCCESS).assignData({room:_room.id,state:_room.state});
						}else{
							_event.setStatus(false).setDescribe(Sundry.describes.UNEXIST).assignData({room:_roomid,err:'room not exist'});
						}
					}else{
						_event.setStatus(false).setDescribe(Sundry.describes.FAIL).assignData({err:'parameters error or miss'});
					}
					fn&&fn(_event);
					socket.send(_event.info);
					break;
